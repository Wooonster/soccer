...
# ---- Logging Functions ----
def setup_logging(log_dir='logs/res50'):
...
def plot_training(log_data, save_dir='logs/res50'):
...
def validate_model(model, dataloader, criterion, device, classes):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(dataloader, desc="Validating"):
            ...
            # Get logits from model output (Hugging Face outputs have .logits attribute)
...
if __name__ == '__main__':
    ...
    log_dir = Path('logs/res50').resolve()
    ...
    # Setup ResNet50 model from Hugging Face
    model = ResNetForImageClassification.from_pretrained(
        "microsoft/resnet-50",  
        num_labels=num_classes,
        ignore_mismatched_sizes=True  # 允许调整最后一层大小
    ).to(device)
